...
public class AdminController {
    ...
    public ResponseEntity<Admin> getById(
            @ApiParam(value = "admin id from which admin object will retrieve", required = true)
    ...
    @PostMapping("/admin")
    public Admin create(
            @ApiParam(value = "admin object store in database table", required = true)
    ...
    public ResponseEntity<Admin> update(
            @ApiParam(value = "admin Id to update admin object", required = true)
            @PathVariable(value = "id") Long id,
            @ApiParam(value = "Update admin object", required = true)
    ...
    public Map<String, Boolean> delete(
            @ApiParam(value = "admin Id from which admin object will delete from database table", required = true)
            @PathVariable(value = "id") Long districtId)
            throws ResourceNotFoundException {
        Admin admin = adminRepository.findById(districtId)
                .orElseThrow(() -> new ResourceNotFoundException("admin not found for this id :: " + districtId));
    ...
    public ResponseEntity<Admin> login(
            @ApiParam(value = "Admin Login", required = true)
            @Valid @RequestBody Login login) {
        Admin admin = adminRepository.getUser(login.getUserName(), login.getPassword());
        if (admin == null) {
